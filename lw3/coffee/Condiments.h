...
{
public:
    explicit CSliceOfChocolate(IBeveragePtr &&beverage, unsigned quantity = 1)
...
protected:
    ...
    std::string GetCondimentDescription() const override
    {
        return std::string(m_type == LiqueurType::Nutty ? "Nutty" : "Chocolate") + " Liqueur";
    }
...
public:
    CSyrup(IBeveragePtr &&beverage, SyrupType syrupType)
            : CCondimentDecorator(move(beverage)), m_syrupType(syrupType)
    {}
...
public:
    CChocolateCrumbs(IBeveragePtr &&beverage, unsigned mass)
            : CCondimentDecorator(move(beverage)), m_mass(mass)
    {
    }
