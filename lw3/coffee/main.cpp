...
int main()
{
    ...
    cout << endl;
    {
        ...
        auto latte = make_unique<CLatte>(CustomCoffeePortion::Standard);
        // добавляем корицы
    ...
    {
        auto beverage =
                make_unique<CSliceOfChocolate>(            // Внешний слой: шоколадная крошка
                        make_unique<CCream>(                // | под нею - кубики льда
                                make_unique<CLiqueur>(            // | | еще ниже лимон
                                        make_unique<CCinnamon>(        // | | | слоем ниже - корица
                                                make_unique<CLatte>(
                                                        CustomCoffeePortion::Double)),    // | | |   в самом сердце - Латте
                                        LiqueurType::Nutty)),            // | 2 кубика сухого льда
    ...
    // Подробнее рассмотрим работу MakeCondiment и оператора <<
    {
        ...
        auto tea = make_unique<CTea>(TeaType::Blue);
        ...
        */
        ...
        auto oneMoreLemonIceTea =
                make_unique<CTea>(TeaType::Red)    // Берем чай
    ...
    {
        auto beverage =
                make_unique<CLatte>(CustomCoffeePortion::Double)                            // Наливаем чашечку латте,
    ...
    {
        auto beverage =
                make_unique<CMilkshake>(MilkshakePortion::Standard)                    // Наливаем молочный коктейль
